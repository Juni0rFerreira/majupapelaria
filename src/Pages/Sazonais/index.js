...
import sazonais from '../../bases/dadosSazonais';
import '../stylecards.css';

function SazonaisPage() {
  ...
  return (
    <div className="App">
      ...
      <div className='container-cards'>
        {sazonais.map((sazonal, index) => (
          <div className='card' key={index}>
            <img
              src={sazonal.imagem || logomaju}
              alt="Item sem imagem"
            />
            <div className='card-body'>
              <h1>{sazonal.nome}</h1>
              <p>{sazonal.descricao}</p>
              <span>R$ {sazonal.valor.toFixed(2).replace('.', ',')}</span>
              <button onClick={() => enviarMensagemWhatsApp(sazonal)}>Entre em contato!</button>
            </div>
          </div>
        ))}
      </div>
